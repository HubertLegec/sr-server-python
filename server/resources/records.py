...
class Records(Resource):
    ...
    def post(self, name):
        ...
        rec_id = self.file_controller.create_record(name, content)
        return jsonify({
    ...
    def put(self, name, record_id):
        req_body = request.get_json()
        content = req_body['content']
        self.file_controller.update_record(name, record_id, content)
        return jsonify({
            'message': 'Record #%d in file %s updated' % (record_id, name)
        })

    def delete(self, name, record_id):
        self.file_controller.delete_record(name, record_id)
        return jsonify({
